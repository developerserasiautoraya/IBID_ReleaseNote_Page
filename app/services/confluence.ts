...
declare const __CONFLUENCE_BASE_URL__: string; 
...
export class ConfluenceApi {
    ...
    this.api = axios.create({
      ...
      headers: {
        ...
        Authorization: `Basic ${btoa(
          `${import.meta.env.VITE_CONFLUENCE_EMAIL}:${import.meta.env.VITE_CONFLUENCE_API_TOKEN}`
        )}`,
      },
    });
  ...
  async getConfluencePageById(pageId: string) {
    const response = await this.api.get(
      `/pages/${pageId}?body-format=styled_view`,
